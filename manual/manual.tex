...
\item \texttt{box{\textunderscore}units} [\texttt{10}] --- number of fundamental unit cells in each cartesian direction. For bcc/fcc this will result in a perfectly cubic box.
...
In the folder containing the distributed code of \texttt{SPECTRA-PKA} there is a ``test'' folder to get the user started. The folder contains the necessary input files to evaluate the PKA distributions of pure zirconium (Zr) in a typical PWR fission spectrum. To run the test navigate to the test folder in the git repository and then, on the command line, type:
\begin{verbatim}
