...
\item \texttt{do{\textunderscore}read{\textunderscore}fispact{\textunderscore}flux{\textunderscore}file} [\texttt{.false.}] --- if set to true then \texttt{SPECTRA-PKA} will, instead, assume that the specified  \texttt{flux{\textunderscore}filename} refers to a \texttt{FISPACT-II}-style ``fluxes'' file -- see the \texttt{FISPACT-II} manual for details. An extensive database of ready-made files are available in the \texttt{FISPACT-II} wiki/github. The group structure of the specified file (i.e. number of energy bins) must be one of the standard structures that \texttt{FISPACT-II} accepts.  
